...
test('repository workflows declare all native release and required security boundaries', async () => {
  ...
  const [
    ...
    read('scripts/import-macos-signing-identity.sh'),
  ...
  assert.match(
    macosSigningScript,
    /security list-keychains -d user -s "\$\{keychain_path\}"/u,
  )
  assert.doesNotMatch(
    macosSigningScript,
    /list-keychains[^\n]*login\.keychain/u,
    'macOS 发布签名不得把宿主登录钥匙串加入搜索范围',
  )
  assert.match(
    macosSigningScript,
    /security find-identity -v -p codesigning "\$\{keychain_path\}"/u,
  )
