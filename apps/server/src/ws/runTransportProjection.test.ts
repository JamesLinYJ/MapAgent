...
describe('run realtime transport projection', () => {
  it('never transports the internal runtime configuration snapshot', () => {
        ...
        sdk: {
          mcp: {
            ...
            servers: [{
              ...
              headers: { 'X-Client-Version': 'runtime-only-version' },
              env: { LOG_LEVEL: 'warn' },
            }],
          },
        },
    ...
    expect(JSON.stringify(projected)).not.toContain('runtime-only-version')
    expect(JSON.stringify(projected)).not.toContain('LOG_LEVEL')
  })
