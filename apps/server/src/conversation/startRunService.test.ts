  ...
  it('resolves a thread session, snapshots principal-scoped runtime config, and starts one detached task', async () => {
    ...
    const service = new StartRunService({
      store: {
        ...
        runtimeConfiguration: { getRuntimeConfig: vi.fn().mockResolvedValue(storedRuntimeConfig(runtimeConfig)) },
    ...
    const service = new StartRunService({
      store: {
        ...
        runtimeConfiguration: {
          getRuntimeConfig: vi.fn().mockResolvedValue(storedRuntimeConfig(defaultRuntimeConfig())),
        },
        getThread: vi.fn(() => ({ sessionId: 'session_goal' } as never)),
        createThread: vi.fn(),
        createRun,
...
function storedRuntimeConfig(config: ReturnType<typeof defaultRuntimeConfig>) {
  return {
    config,
    revision: 1,
