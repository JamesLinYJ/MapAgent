...
import { ProviderCredentialStagingService } from '../model/customProviderCredentials.js'
...
export async function createAppContainer(input: {
  ...
  const runtimeDomainJournal = new PostgresRunDomainJournalRepository(db)
  const stepContexts = new AgentStepContextFactory(
    ...
    new PostgresContextWindowRepository(db, runtimeDomainJournal),
  )
