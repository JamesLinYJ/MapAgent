...
import type { RunModelUsageRecorder } from './runModelUsageRecorder.js'
...
export interface SubAgentRuntimeDependencies {
  ...
  modelUsageRecorder: RunModelUsageRecorder
...
export function createSubAgentDeliveryAgent(
  ...
  return new Agent<AgentsExecutionContext, typeof subAgentDeliverySchema>({
    ...
    // Agent-as-tool 使用独立的嵌套 Runner，其 rawResponses 不会
    // 进入父 stream；在这个专有边界入账才能既不漏记也不重复。
    model: dependencies.modelUsageRecorder.ownNestedModel(
      resolveSubAgentModel(config, dependencies),
    ),
