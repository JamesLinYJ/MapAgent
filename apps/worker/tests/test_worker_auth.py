...
class NonceCacheCapacityTests(unittest.TestCase):
    ...
    def test_overflow_rejects_new_nonce_without_eviction(self) -> None:
        """超过上限时拒绝新请求，不能淘汰仍然有效的 nonce。"""
        now = int(time.time())
        # 填充 5 个 nonce，exp 递增
        for i in range(5):
            nonce = f"keep-me-{i}"
        ...
        err = self.verifier.verify(auth, self.tool, self.body)
        self.assertEqual(err, (503, "Worker nonce 存储已满，请稍后重试"))
        self.assertEqual(len(self.verifier.seen_nonces), 5)
