...
import { MacosInstalledCliService } from './macosInstalledCliService.js'
...
const desktopEnvironment = projectDesktopEnvironment(process.env)
...
async function startDesktop(logger: DesktopSystemLogger): Promise<void> {
  ...
  const packagedLocalRuntime = app.isPackaged
    ? await preparePackagedLocalRuntime({
        ...
        systemRuntimeManifestPath: defaultDesktopRuntimeManifestPath(process.platform, desktopEnvironment),
      })
    : null
  ...
  const installedCli = new MacosInstalledCliService({
    platform: process.platform,
